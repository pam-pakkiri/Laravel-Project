<div>
      ...
      <form wire:submit.prevent="onBinaryToAscii">
        ...
        <div class="input-group input-group-flat mb-3">
            <textarea class="form-control" wire:model.defer="binary" placeholder="{{ __('Paste your Binary code here...') }}" rows="10" required></textarea>
            <span class="input-group-text {{ ( Cookie::get('theme_mode') == 'theme-light' ) ? 'bg-white' : '' }}">
                <a id="paste" class="btn btn-icon btn-success cursor-pointer position-absolute top-0 end-0 m-2" title="{{ __('Paste') }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Paste') }}">
